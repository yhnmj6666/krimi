<template>
  <v-container style="height:100%">
      ...
      <v-col md="9">
        <v-row>
          ...
          >
            ...
            <v-card
              :style="{
                transform: `rotate(${parseInt(1 - Math.random() * 2)}deg)`,
              }"
            >
              <v-card-text>
                ...
                <v-chip-group column>
                  <v-chip
                    ...
                    v-on:mouseenter="loadExampleImage(mean)"
                    v-on:mouseover="hoverWord = true"
                    v-on:mouseleave="hoverWord = false"
    ...
    <img v-if="hoverWord" :src="wordImageUrl" class="hintImage" :style="{ left: imgX + 'px', top: imgY + 'px' }"/>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    async loadExampleImage(word) {
      this.wordImageUrl = await this.$store.dispatch("getWordUrl", {
        word: word,
      });
    },
    onMouseMove(e) {
      this.imgX = e.pageX;
      this.imgY = e.pageY - 513;
    }
  },
  mounted() {
